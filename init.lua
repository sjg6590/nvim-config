...
local cursor_state = { buf = nil, win = nil }

local function cursor_open_float()
  local width = math.floor(vim.o.columns * 0.85)
  local height = math.floor(vim.o.lines * 0.85)
  local row = math.floor((vim.o.lines - height) / 2)
  local col = math.floor((vim.o.columns - width) / 2)
...
local function cursor_toggle()
  if cursor_state.win and vim.api.nvim_win_is_valid(cursor_state.win) then
    vim.api.nvim_win_hide(cursor_state.win)
    cursor_state.win = nil
    return
  end
  if cursor_state.buf and vim.api.nvim_buf_is_valid(cursor_state.buf) then
    local _, win = cursor_open_float()
    vim.api.nvim_win_set_buf(win, cursor_state.buf)
    cursor_state.win = win
    vim.cmd 'startinsert'
    return
  end
  local buf, win = cursor_open_float()
  cursor_state.buf = buf
  cursor_state.win = win
  vim.fn.termopen('cursor-agent', {
    cwd = vim.fn.getcwd(),
    on_exit = function()
      cursor_state.buf = nil
      cursor_state.win = nil
    end,
  })
  vim.cmd 'startinsert'
...
local function cursor_send_file()
  ...
  if not (cursor_state.win and vim.api.nvim_win_is_valid(cursor_state.win)) then
    cursor_toggle()
  end
  vim.api.nvim_chan_send(vim.bo[cursor_state.buf].channel, 'Review @' .. path .. '\n')
end
...
local function cursor_send_selection()
  ...
  vim.api.nvim_chan_send(vim.bo[cursor_state.buf].channel, text)
end

local function cursor_send_diagnostics()
  local bufnr = vim.api.nvim_get_current_buf()
  local path = vim.api.nvim_buf_get_name(bufnr)
  if path == '' then
    vim.notify('No file open', vim.log.levels.WARN)
  ...
  vim.api.nvim_chan_send(vim.bo[cursor_state.buf].channel, msg)
end

vim.keymap.set('n', '<leader>uc', cursor_toggle, { desc = 'Toggle [C]ursor Agent' })
